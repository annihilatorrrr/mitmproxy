    ...
    items.extend(
        (
            ...
            (
                "weight",
                0.5,
                TruncatedText(domain, colorize_host(domain), "right"),
            ),
            fcol(f"({fixlen(type, 5)[:len(type)]}) =", "text"),
            (
                "weight",
                1,
                truncated_plain(
                    "..." if answer is None else answer or "?", "text"
                )
    ...
    *,
    ...
    if isinstance(f, (TCPFlow, UDPFlow)):
        ...
        if f.messages:
            ...
            duration = None
        protocol = "quic" if f.client_conn.tls_version == "QUIC" else f.type
